...
import { fetchCharacter, resetCharacterState } from '../../redux/characterSlice'; // Import the reset action
...
function Home() {
  ...
  const handleSearch = () => {
    if (name.trim()) {
      dispatch(fetchCharacter(name)); // Dispatch the thunk
    }
  };
  ...
  const handleLogout = () => {
    if (window.confirm('Do you want to logout?')) {
      dispatch(logout());
      dispatch(resetCharacterState()); // Reset character state on logout
      navigate('/login');
    }
  };
